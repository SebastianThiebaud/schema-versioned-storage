...
describe('createPersistedState', () => {
  ...
  it('should handle storage.setItem error', async () => {
    ...
    await storage.init();
    await expect(storage.set('name', 'test')).rejects.toThrow('Storage write error');
  });
  ...
  it('should ensure _version is set on state', async () => {
    ...
    await adapter.setItem('test-storage', JSON.stringify({ name: 'test', count: 0 }));
  ...
  it('should throw error when set is called before init', async () => {
    await expect(storage.set('name', 'test')).rejects.toThrow('not initialized');
  });

  it('should throw error when update is called before init', async () => {
    await expect(storage.update('name', (prev) => prev)).rejects.toThrow('not initialized');
  });
