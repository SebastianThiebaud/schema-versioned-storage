...
export default migration;`;
    ...
    await writeFile(
      join(migrationsDir, "2-test-migration.ts"),
      migrationContent,
    );
    ...
    expect(indexContent).toContain(
      "import migration0 from './2-test-migration'",
    );
...
export default migration;`,
    ...
    const import1Index = indexContent.indexOf(
      "import migration0 from './1-initial-migration'",
    );
    const import2Index = indexContent.indexOf(
      "import migration1 from './2-first-migration'",
    );
    const import3Index = indexContent.indexOf(
      "import migration2 from './3-second-migration'",
    );
  ...
  it("should use custom types path", async () => {
    ...
    const indexContent = await readFile(indexPath, "utf-8");
    expect(indexContent).toContain(
      "import type { Migration } from './custom-types'",
    );
  });
  ...
  it("should use config file with nested format", async () => {
    ...
    const indexContent = await readFile(indexPath, "utf-8");
    expect(indexContent).toContain(
      "import type { Migration } from 'custom-package'",
    );
  });
  ...
  it("should use config file with flat format", async () => {
    ...
    const indexContent = await readFile(indexPath, "utf-8");
    expect(indexContent).toContain(
      "import type { Migration } from 'flat-package'",
    );
  });
  ...
  it("should use package.json configuration", async () => {
    ...
    const indexContent = await readFile(indexPath, "utf-8");
    expect(indexContent).toContain(
      "import type { Migration } from 'package-json-types'",
    );
  });
  ...
  it("should handle CLI args overriding package.json", async () => {
    ...
    const indexContent = await readFile(indexPath, "utf-8");
    expect(indexContent).toContain(
      "import type { Migration } from 'cli-override-types'",
    );
  });
