 ...
 */
export function loadPackageJsonConfig(cwd: string = process.cwd()): Partial<SchemaHashesConfig> | null {
  try {
 ...
 */
export function parseArgs(args: string[] = process.argv.slice(2), cwd: string = process.cwd()): SchemaHashesConfig {
  // Start with defaults
